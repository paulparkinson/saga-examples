...
import com.oracle.saga.bankA.data.*;
...
public class AccountsService {
    ...
    private String newInAccountsTable(OracleJsonObject account) {
        ...
        long insertedId=-1;
                ...
                try (OraclePreparedStatement stmt1 = (OraclePreparedStatement) connection.prepareStatement(newEntryBankAccount)) {
                    ...
                    logger.error("INSERT IN ACCOUNTS Bank A ERROR", ex);
                }
    ...
    private boolean validateUserWithdrawing(OracleJsonObject accountsJsonObj) {
        ...
        String select_CC = "SELECT COUNT(*) from bankA where ucid = ? and account_number = ?";
        int rslt = 0;
        int count=-1;
        ...
        try (OraclePreparedStatement stmt = (OraclePreparedStatement) connection.prepareStatement(select_CC)) {
            ...
            logger.error("Validate Account Bank A error", ex);
        }
    ...
    public String selectAllAccounts(OracleJsonObject accountsJsonObj){
        String select_CC = "SELECT account_number, account_type, balance_amount, created_at from bankA where ucid = ? and account_type = ?";
        int rslt = 0;
        JsonArrayBuilder jsonArrayBuilder = Json.createArrayBuilder();
        try (OraclePreparedStatement stmt = (OraclePreparedStatement) connection.prepareStatement(select_CC)) {
            stmt.setString(1, accountsJsonObj.get("ucid").toString().replaceAll(REG_EXP_REMOVE_QUOTES, ""));
    ...
    public static String viewAllAccounts(Connection connection, ViewBADTO payload, String type){
        String select_BAC = "SELECT account_number, account_type, balance_amount, created_at from bankA where ucid = ? and account_type = 'CHECKING'";
        String select_BAS = "SELECT account_number, account_type, balance_amount, created_at from bankA where ucid = ? and account_type = 'SAVING'";
        String select_CC = "SELECT account_number, account_type, balance_amount, created_at from bankA where ucid = ? and account_type = 'CREDIT_CARD'";
        int rslt = 0;
    ...
    public boolean updateOperationStatus(String sagaId, String state){
        ...
        try (PreparedStatement stmt = connection.prepareStatement(updateOperationStateBooking)) {
            ...
            logger.error("UPDATE STATUS from ACCOUNTS_BOOK Bank A error", ex);
        }
        return rslt > 0;
    }
    ...
    public boolean updateAccountNumberInLogs(String sagaId, String account_number){
        ...
        try (PreparedStatement stmt = connection.prepareStatement(updateOperationStateBooking)) {
            ...
            logger.error("UPDATE Account number in ACCOUNTS_BOOK Bank A error", ex);
        }
        return rslt > 0;
    }

    public boolean updateOperationStatus(String sagaId, String state, String txnType){
        String updateOperationStateBooking = "UPDATE bankA_book set operation_status = ? where SAGA_ID = ? and transaction_type = ?";
        int rslt = 0;
        try (PreparedStatement stmt = connection.prepareStatement(updateOperationStateBooking)) {
            stmt.setString(1, state);
            stmt.setString(2, sagaId);
    ...
    private int insertInBook(OracleJsonObject account, String sagaId) {
        ...
        String insertCustomerInfo = "INSERT INTO bankA_book (saga_id, ucid, operation_type, transaction_type, transaction_amount, account_number,operation_status) VALUES (?,?,?,?,?,?,?)";
        int insertRslt = 0;
    ...
    public boolean deleteAccount(CompensationData account) {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(deleteAccount)) {
            ...
            logger.error("UNABLE TO DELETE ACCOUNT FROM ACCOUNTS TABLE Bank A", ex);
        }
        return rslt > 0;
    }

    public static boolean bankCheck(Connection connection, BankCheckDTO payload){
        String deleteAccount = "Select count(*) FROM bankA where ACCOUNT_NUMBER = ?";
        int rslt = 0;
        Boolean validateStatus = Boolean.FALSE;
        try (PreparedStatement stmt = connection.prepareStatement(deleteAccount)) {
            stmt.setString(1, payload.getAccount_number());
            ...
            logger.error("UNABLE TO VERIFY ACCOUNT FROM ACCOUNTS TABLE Bank A", ex);
        }
    ...
    public static String getNotifications(Connection connection){
        ...
        String selectOngoingNotificationQuery = "SELECT saga_id,ucid,operation_type,operation_status from bankA_book where operation_status = 'ONGOING'";
        String getNotificationQuery = "UPDATE bankA_book SET read = 'TRUE' WHERE read = 'FALSE' and operation_status != 'PENDING' and operation_status != 'ONGOING' RETURNING saga_id,ucid,operation_type,operation_status into ?,?,?,?";
        int rowsAffected = 0;
